    ...
    <div className="min-h-screen bg-black pt-32 pb-24">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-14">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 border border-white/15 text-white/80 mb-6"><Sparkles className="w-4 h-4 text-[#F5D76E]" /> Custom Orlando event packages</div>
          <h1 className="text-5xl md:text-6xl text-white mb-6">Photo Booth <span className="opbe-gold-text">Packages</span></h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {packages.map((pkg, index) => (
            <Card key={pkg.id} className={`opbe-premium-card opbe-premium-card-hover ${index === 1 ? 'border-[#D4AF37]/60 shadow-2xl shadow-[#D4AF37]/20' : ''} rounded-3xl p-8 flex flex-col`}>
              <h2 className="text-2xl text-white mb-2">{pkg.name}</h2>
              <p className="text-[#F5D76E] mb-4">{pkg.price}</p>
              <p className="text-white/60 mb-6">Ideal for {pkg.idealFor}. Recommended duration: {pkg.duration}.</p>
              <div className="space-y-3 mb-8 flex-1">
                {pkg.features.map((feature) => <div key={feature} className="flex gap-3 text-white/75"><Check className="w-5 h-5 text-[#F5D76E] flex-shrink-0" />{feature}</div>)}
              </div>
              <p className="text-white/50 text-sm mb-6">Popular add-ons: {pkg.addOns.join(', ')}</p>
              <Button asChild className="opbe-btn-primary py-6">

    ...
    <div className="min-h-screen bg-black pt-32 pb-24">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_420px] gap-10 items-center mb-20">
          <div>
            <p className="text-purple-300 mb-4 uppercase tracking-[0.3em] text-sm">Oralndo Ohiti Booth Events</p>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Button asChild className="bg-gradient-to-r from-purple-500 to-pink-500 text-white px-8 py-6 rounded-full"><Link to="/booking">Book Your Booth <ArrowRight className="w-5 h-5 ml-2" /></Link></Button>
          ...
          <Card className="bg-white/10 border-white/15 rounded-3xl p-8 backdrop-blur-xl">
            <h2 className="text-2xl text-white mb-5">Why Orlando events choose us</h2>
            {['Fast quote response', 'Custom event packages', 'Digital sharing included', 'Premium backdrops and lighting', 'Wedding, party, and corporate-ready service'].map((item) => <div key={item} className="flex gap-3 text-white/75 mb-4"><CheckCircle2 className="w-5 h-5 text-purple-300 flex-shrink-0" />{item}</div>)}
          </Card>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-20">
          {['Premium guest experience', 'Orlando-focused service', 'Share-ready content'].map((title) => (
            <Card key={title} className="bg-white/5 border-white/10 rounded-3xl p-6">
        ...
        <Card className="bg-gradient-to-r from-purple-600 to-pink-600 border-0 rounded-3xl p-10 text-center mb-20">
          <h2 className="text-3xl text-white mb-4">Plan your {page.service.toLowerCase()} with Oralndo Ohiti Booth Events</h2>
          ...
          <Button asChild className="bg-white text-purple-700 hover:bg-white/90 rounded-full px-8 py-6"><Link to="/quote">Request My Quote</Link></Button>
        </Card>

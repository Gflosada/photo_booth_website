    ...
    <header
      className={`fixed top-0 left-0 right-0 z-50 border-b transition-all duration-300 ${
        isScrolled
          ...
          : 'border-white/5 bg-black/55 backdrop-blur-md md:border-transparent md:bg-transparent md:backdrop-blur-0'
      }`}
    >
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" aria-label="Main navigation">
        <div className="flex h-16 items-center justify-between sm:h-20">
          ...
          <div className="hidden lg:flex items-center gap-7">
            {navItems.map((item) => (
              <NavLink
                key={item.to}
                to={item.to}
                className={({ isActive }) => `text-sm transition-colors relative ${isActive ? 'text-[#D4AF37]' : 'text-white/70 hover:text-white'}`}
              >
                {({ isActive }) => (
                  <>
                    {item.label}
                    {isActive && <span className="absolute -bottom-1 left-0 right-0 h-0.5 bg-gradient-to-r from-[#D4AF37] to-[#B8860B]" />}
                  </>
                )}
              </NavLink>
            ))}
          ...
          <div className="hidden md:flex items-center gap-3">
            <Button asChild variant="outline" className="opbe-btn-secondary rounded-full">
              <Link to="/quote" onClick={() => trackQuoteClick({ location: 'header' })}>Get Quote</Link>
            </Button>
            <Button asChild className="opbe-btn-primary rounded-full shadow-lg shadow-[#D4AF37]/30">
          ...
          <button
            ...
            className="lg:hidden text-white focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#F5D76E] rounded-md"
          >
          ...
          <div className="lg:hidden py-4 border-t border-white/10 bg-black/90 backdrop-blur-xl">
            {navItems.map((item) => (
              <NavLink
                ...
                className={({ isActive }) => `block w-full px-4 py-3 text-sm transition-colors rounded-xl ${isActive ? 'text-[#D4AF37] bg-white/5' : 'text-white/70 hover:text-white hover:bg-white/5'}`}
              >
                {item.label}
              </NavLink>
            ))}
            <div className="grid grid-cols-2 gap-3 px-4 pt-4">
              ...
              <Button asChild className="rounded-full opbe-btn-primary">
